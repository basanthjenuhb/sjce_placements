<?php
	...
	if(isset($_POST['cname']))
	{
		...
		$query2=mysql_query("SELECT sslc,puc,ug,pg,type FROM company WHERE name LIKE '{$_POST['cname']}'");
		if($query2)
		{
			if(mysql_num_rows($query2)==1)
			{
				...
				{
					if($row3['type']=='opendream' && $row3['opendream']!='')
						die("INVALID DETAILS");
					if($row3['type']=='dream')
					{
					...
					{
						if($row0['opendream']!='' || $row0['dream']!='' || $row0['core']!='' || $row0['mass']!='' || $row0['openmass']!='')
							die("INVALID DETAILS");
					}
				}
				else
